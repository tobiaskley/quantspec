...
%%\VignetteIndexEntry{quantspec Introduction}
...
\title{An Object-oriented Framework for\\ Quantile-based Spectral Analysis\\ and a Reference Implementation in \proglang{R}:\\ The \pkg{quantspec} Package}
...
\Abstract{Quantile-based spectral approaches to the analysis of time series have
recently attracted a lot of attention. In several publications the advantages of
copula-based measures for serial dependence over the traditional moment-based
ones have been discussed and two different types of estimators were proposed.
The asymptotic properties of these estimators were subject to many a detailed
analysis with the results published in several scientific papers. In this paper
aspects of their systematic computation are investigated. An extensible
framework is developed and documented using object-oriented models. As a
comprehensive reference implementation the \proglang{R} package \pkg{quantspec}
was made available and the contribution in this paper includes a tutorial, with
worked examples. A reader who is already familiar with quantile-based spectral analysis and whose primary interest is not the design of the \pkg{quantspec} package, but how to use it,
can read the tutorial and worked examples (Sections~\ref{sec:3} and~\ref{sec:4})
independently.

This introduction to the \proglang{R} package \pkg{quantspec} is
a (slightly) modified version of a paper (to be) submitted to the \emph{Journal
...
and a similar representation for $\gamma_k^U(\tau_1, \tau_2)$, they are equivalent to the ``time-domain'' quantities.
...
The following quantile periodogram statistics can be used for the estimation of the Laplace and copula-spectra defined in Section~\ref{sec:Lcck}.
For the upcoming definitions denote by $X_0, \ldots, X_{n-1}$ an observed time
series of the process $(X_t)_{t \in \IZ}$, and by
\[\hat F_n(x) := \sum_{t=0}^{n-1} I\{X_t \leq x\}\]
the \emph{empirical distribution function} of $X_0, \ldots, X_{n-1}$.
Further,
...
\begin{defi}[Quantile-regression based peridograms]
  \label{def:qregPG} ~\\
  For $\omega \in \IR$ and $\tau_1, \tau_2 \in (0,1)$ the
  \emph{Laplace peridogram} $\hat L_{n}^{\tau_1, \tau_2}(\omega)$, and the \emph{rank-based Laplace periodogram} $\hat L_{n,R}^{\tau_1,\tau_2}(\omega)$ are defined as
  \begin{equation*}
    \hat L_{n}^{\tau_1, \tau_2}(\omega) := (2\pi n)^{-1} \hat b_{n}^{q_1}(\omega) \hat b_{n}^{q_2}(-\omega), \quad
    \hat L_{n,R}^{\tau_1,\tau_2}(\omega) := (2\pi n)^{-1} \hat b_{n,R}^{\tau_1}(\omega) \hat b_{n,R}^{\tau_2}(-\omega),
  \end{equation*}
  where, for $\omega \neq 0 \mod \pi$, and $\tau\in (0,1)$,
  \begin{align}
  (\hat a^{\tau}(\omega), \hat b_{n}^{q}(\omega)) & := \argmin_{a \in \IR, b \in \IC} \sum_{t=0}^{n-1} \rho_{\tau}(n X_{t-1} - a - 2 \cos(\omega t) \Re b - 2 \sin(\omega t) \Im b), \label{L1regre} \\
  (\hat a^{\tau}(\omega), \hat b_{n,R}^{\tau}(\omega)) & := \argmin_{a \in \IR, b \in \IC} \sum_{t=0}^{n-1} \rho_{\tau}(n \hat F_n(X_t) - a - 2 \cos(\omega t) \Re b - 2 \sin(\omega t) \Im b).\nonumber
  \end{align}
  ...
  \begin{align}
  (\hat a^{\tau}(\omega_{\pi}), \hat b_{n}^{q}(\omega_{\pi})) & := \argmin_{a \in \IR, b \in \IR} \sum_{t=0}^{n-1} \rho_{\tau}(n X_t - a - \cos(\omega_{\pi} t) b), \nonumber \\
  (\hat a^{\tau}(\omega_{\pi}), \hat b_{n,R}^{\tau}(\omega_{\pi})) & := \argmin_{a \in \IR, b \in \IR} \sum_{t=0}^{n-1} \rho_{\tau}(n \hat F_n(X_t) - a - \cos(\omega_{\pi} t) b).\nonumber
  \end{align}
\end{defi}
...
The Laplace periodograms trace back to \cite{Li2008} who, in a pioneering paper,
suggested least absolute deviation estimators in a harmonic linear model. He
later extended the approach to arbitrary quantiles \cite{Li2012}.
\cite{DetteEtAl2013} introduced the estimator with distinct quantile levels $\tau_1$ and $\tau_2$ (not necessarily equal),
and based it on the ranks.

\begin{defi}[Peridograms based on clipped time series]
  \label{def:clippedPG} ~\\
  For $\omega \in \IR$ and $q_1, q_2 \in \IR$, the
  \emph{clipped time-series peridogram} is defined as
  \begin{equation*}
    I_{n}^{q_1, q_2}(\omega) := (2\pi n)^{-1} d_{n}^{q_1}(\omega) d_{n}^{q_2}(-\omega), \quad
    d_{n}^{q}(\omega) := \sum_{t=0}^{n-1} I\{X_t \leq q\} {\rm e}^{-{\rm i} \omega t}.
  \end{equation*}
  For $\omega \in \IR$ and $\tau_1, \tau_2 \in [0,1]$ the \emph{copula rank periodogram} is defined as
  \begin{equation*}
    \label{eq:inr}
...
Note the similarity between all the periodogram-like estimators and the cross-perio\-do\-grams in multivariate time series analysis [cf., e.\,g., \cite{Brillinger1975}, p.\,235]: each periodogram is a product of two frequency representation objects computed at frequencies ($\omega$ and $-\omega$) that sum to zero.

The estimator based on the discrete
Fourier transformation of $I\{\hat F_n(X_t) \leq \tau\}$
was introduced by \cite{Hagemann2011}, for the special
case of $\tau_1 = \tau_2$. The case of distinct quantile levels $\tau_1$ and
$\tau_2$ (not necessarily equal) was discussed in \cite{KleyEtAl2014}, where
weak convergence to a Gaussian process is proven.

\subsection{Smoothing the quantile periodograms}
\label{sec:SmEstrs}

As in the traditional case the new periodograms are not consistent estimators [cf. the positive variances of the limit distributions in Theorems~3.2 and~3.4 in \cite{DetteEtAl2013} or Proposition~3.4 in \cite{KleyEtAl2014}]. To achieve consistency of the estimators we convolve the sequence of periodograms (indexed with the Fourier frequencies) with a sequence of weighting functions $W_n$. When the weight functions are such that with $n \rightarrow \infty$ only the weights in a shrinking neighborhood of zero will be positive, the estimator will be consistent.

The smoothed periodograms are defined as follows:

\begin{defi}[Smoothed quantile-regression based peridograms]\label{def:SmqregPG} ~\\
  For $\omega \in \IR$ and $\tau_1, \tau_2 \in (0,1)$ the \emph{smoothed Laplace peridogram} $\hat{\mathfrak{f}}_{n}(\tau_1, \tau_2; \omega)$ and \emph{smoothed rank-based laplace peridogram} $\hat{\mathfrak{f}}_{n,R}(\tau_1, \tau_2; \omega)$ are defined as
  \begin{equation*}
  \begin{split}
...
\begin{defi}[Smoothed peridograms based on clipped time series]\label{def:SmclippedPG} ~\\
  For $\omega \in \IR$ and $q_1, q_2 \in \IR$ the \emph{smoothed clipped time series peridogram} is defined as
  \begin{equation*}
    \hat G_{n}(q_1, q_2; \omega) := \frac{2\pi}{n} \sum_{s=1}^{n-1} W_n\big( \omega - 2\pi s / n \big) I_{n}^{q_1, q_2}(2 \pi s / n).
  \end{equation*}
  For $\omega \in \IR$ and $\tau_1, \tau_2 \in [0,1]$ the \emph{smoothed copula rank peridogram} is defined as
  \begin{equation*}
    \hat G_{n,R}(\tau_1, \tau_2; \omega) := \frac{2\pi}{n} \sum_{s=1}^{n-1} W_n\big( \omega - 2\pi s / n \big) I_{n,R}^{\tau_1, \tau_2}(2 \pi s / n).
  \end{equation*}
\end{defi}

A comprehensive description of all estimators and their asymptotic properties is
available in \cite{Kley2014a}.
...
where $F \subset \mathbb{R}$ is a set of frequencies [e.\,g, $F = [0,2\pi)$] and $T_1, T_2 \subset \bar{\mathbb{R}}$ are sets of levels. To provide a common interface to these objects the abstract class \code{QSpecQuantity} was introduced. It's data fields (i.\,e., the array \code{values}, a vector of reals \code{frequencies} and a list with two vectors of reals \code{levels}), are designed to store the sets
\begin{equation*}
\begin{split}
...
\footnote{The function show is used for printing object's of this class, and all superclasses, to the console.}
...
\begin{figure}[t]
  \centering
  \scalebox{1.2}{\includegraphics{../man/figures/main}}
...
\end{split}
\end{equation}
respectively. In the framework this fact is incorporated by introduction of the abstract class \code{FrequencyRepresentation} and its two subclasses \code{ClippedFT} and \code{QRegEstimator}, where the actual computations are implemented via the method \code{initialization()}. The class \code{FrequencyRepresentation} serves as a common interface to the quantities in~\eqref{eqn:FR}. It provides data fields to store various information, including
\begin{itemize}
...
\begin{figure}[t]
  \centering
  \scalebox{1.2}{\includegraphics{../man/figures/csd}}
...
example is to invoke the accessor function \code{getValues}, which is equiped
...
normally distributed random variales execute the following lines on the
\proglang{R} shell:
...
Several examples explaining on how to use the
various functionality can be found in the online help files or the folder
\code{examples} in the directory where the package is installed. The package
...
Of course, the second line initiates plotting the smoothed CR peridogram, which
...
plot. A complete description of the options available is available in the online
...
\code{demo("qar-simulation")} and by refering to the function \code{ts1}, which
...
where $U_t$ is independent white noise with $U_t \sim \mathcal{U}[0,1]$
...
pointing out that in this example the estimator performs already quite well.
...
Setting the seed in the very beginning allows for reproducable results.
...
the estimator can be analysed. Examples include the bias,
...
These numbers could now be inspected to, for example, observe that the smoothed
...
function to perform quantile spectral analysis for locally stationary processes,
the computation and smoothing of higher order quantile periodograms for
the estimation of quantile polyspectra. Procedures for graphical representations
of these object, possibly animated ones, will amend these planned parts of the
...
deliberately designed in an object-oriented and extensible design. Thus it is
...
modelling of nonlinear dynamic processes'' (SFB 823) of the Deutsche
Forschungsgemeinschaft and by a PhD Grant of the Ruhr-Universit\"{a}at Bochum
