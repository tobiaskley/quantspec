...
distribution-based measures for serial dependence over the traditional moment-based
...
framework for quantile-based spectral analysis of time series is developed and
documented using object-oriented models. A comprehensive open source reference
implementation, the \proglang{R} package \pkg{quantspec}, was recently made
available and the contribution in this paper includes a tutorial, with worked
examples, on how it can be used. A reader who is already familiar with
quantile-based spectral analysis and whose primary interest is not the design of
the \pkg{quantspec} package, but how to use it, can read the tutorial and worked
examples (Sections~\ref{sec:3} and~\ref{sec:4}) independently.
...
Attempts to robustify the traditional spectral methods against
outliers and deviations from the distributional assumptions were taken several
times [cf.
...
than would the autocovariances. As in the approach of \cite{Hong1999} a complete
description of the joint distributions (or copulas) of the pairs $(X_t, X_{t-k})$
is available. In addition to the generalized spectrum of \cite{Hong1999}, the
quantile-based appraoch can be more robust than traditional spectral analysis,
too. Another feature that can be quite handy in many applications is the
invariance of the copula cross-covariance kernel to monotone transformations,
which in particular disentangles the serial features from the marginal features.
The full amount of advantages for considering these measures is too extensive to
be discussed here in full detail and further arguments are omitted since
thorough discussions are already available in \cite{DetteEtAl2013} and
\cite{KleyEtAl2014}.

Under sumability conditions of $(\gamma_k)$ and $(\gamma_k^U)$ the
...
\end{equation}
where $q_{\tau} := F^{-1}(\tau)$. Admitting the representation
...
representations in the ``frequency domain'' are equivalent to the
...
Recently, there has been a renewed surge of interest in that type of concept,
...
[cf. \cite{DetteEtAl2013,Hagemann2011,Kley2014a,KleyEtAl2014,LeeRao2012,Li2008,Li2012,Li2013,Li2014}].
...
In this section of the introduction, statistics (quantile periodograms) that can
be used for the estimation of the Laplace and copula-spectra in Section~\ref{sec:Lcck} are
defined.
...
\begin{defi}[Quantile-regression based periodograms]
  ...
  \begin{align}
  (\hat a_{n}^{\tau}(\omega), \hat b_{n}^{\tau}(\omega)) & := \argmin_{a \in \IR, b \in \IC} \sum_{t=0}^{n-1} \rho_{\tau}(n X_{t-1} - a - 2 \cos(\omega t) \Re b + 2 \sin(\omega t) \Im b), \label{L1regre} \\
...
$\tau_1$ and $\tau_2$ (not necessarily equal), and based it on the ranks.
...
pointing out that in this example ($N=512$) the estimator performs already
quite well.
